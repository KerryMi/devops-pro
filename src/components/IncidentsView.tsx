  ...
  if (currentScenario) {
    ...
    return (
      <div className="min-h-screen space-y-6 pb-20 animate-fadeIn text-slate-900 dark:text-slate-100">
        ...
        <div className="sticky top-2 z-30 bg-white/90 dark:bg-[#0f172a]/90 backdrop-blur-md border border-slate-200 dark:border-slate-800 p-3.5 rounded-2xl shadow-lg flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center space-x-3 min-w-0">
            <button
              onClick={() => setActiveScenarioId(null)}
              className="px-3.5 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-xs font-bold transition-all flex items-center space-x-2 shrink-0 cursor-pointer"
            >
            ...
            <div className="flex items-center space-x-2 min-w-0">
              ...
              <h3 className="text-xs sm:text-sm font-extrabold truncate text-slate-900 dark:text-white max-w-[200px] sm:max-w-[350px] lg:max-w-[500px]">
          ...
          <div className="flex items-center space-x-2 shrink-0">
            {isSolved ? (
              <span className="px-3 py-1 rounded-xl bg-emerald-500/15 border border-emerald-500/30 text-emerald-600 dark:text-emerald-400 text-xs font-extrabold flex items-center space-x-1.5">
                <CheckCircle2 className="w-4 h-4" />
                <span>Ликвидирован</span>
              </span>
            ) : (
              <span className="px-3 py-1 rounded-xl bg-rose-500/15 border border-rose-500/30 text-rose-600 dark:text-rose-400 text-xs font-extrabold flex items-center space-x-1.5">
            ...
            <div className="flex items-center space-x-1 border-l border-slate-200 dark:border-slate-800 pl-2 ml-1">
              <button
                disabled={currentScenarioIndex <= 0}
                onClick={handlePrevScenario}
                className="p-2 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 disabled:opacity-30 disabled:cursor-not-allowed text-slate-700 dark:text-slate-300 transition-colors cursor-pointer"
        ...
        <div className="flex items-center space-x-2 border-b border-slate-200 dark:border-slate-800 pb-1">
          <button
            ...
            className={`px-5 py-3 rounded-2xl text-xs sm:text-sm font-extrabold transition-all flex items-center space-x-2 cursor-pointer ${
          ...
          >
            ...
            <span>1. Диагностика в консоли ({executedCommands.length}/{currentScenario.diagnosticSteps.length})</span>
          </button>
          ...
          >
            ...
            <span>2. Устранение аварии (Playbook)</span>
            {selectedFixId && (
          ...
          {showSolution && (
            ...
            >
              ...
              <span>3. Post-Mortem & Первопричина</span>
            </button>
          )}
        </div>
        ...
        {activeTab === 'diagnostics' && (
          <div className="space-y-5 animate-fadeIn">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white dark:bg-[#121927] p-4 rounded-2xl border border-slate-200 dark:border-slate-800">
              <div className="space-y-0.5">
                <h3 className="text-sm font-black text-slate-900 dark:text-white flex items-center space-x-2">
                  <Terminal className="w-4 h-4 text-emerald-500" />
                  <span>Интерактивный Терминал Дежурного Инженера</span>
                </h3>
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  Запускайте диагностические команды по очереди, чтобы выявить первопричину сбоя в системе.
                </p>
              </div>
              ...
              <div className="flex items-center space-x-2">
                <button
                  ...
                  className="px-3.5 py-2 rounded-xl bg-emerald-500/15 hover:bg-emerald-500/25 border border-emerald-500/30 text-emerald-700 dark:text-emerald-300 text-xs font-bold transition-all flex items-center space-x-1.5 cursor-pointer shrink-0"
                >
                  <Play className="w-3.5 h-3.5 fill-current" />
                  <span>Выполнить все команды</span>
                </button>
              </div>
            </div>
            ...
            <div className="p-5 rounded-2xl bg-white dark:bg-[#121927] border border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="space-y-0.5">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  {allDiagnosticsRun 
                    ? 'Все диагностические команды выполнены! Переходите к выбору действия по устранению сбоя.'
                    : 'Вы можете запустить оставшиеся команды или перейти к исправлению сразу.'}
                </p>
              </div>

              <button
                onClick={() => setActiveTab('fix')}
                className="px-6 py-3 rounded-xl bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-slate-950 font-black text-xs sm:text-sm tracking-wide transition-all shadow-md shadow-amber-500/20 flex items-center justify-center space-x-2 shrink-0 cursor-pointer"
              >
                <span>Перейти к исправлению (Playbook)</span>
        ...
        {activeTab === 'fix' && (
          <div className="space-y-5 animate-fadeIn">
            <div className="bg-white dark:bg-[#121927] p-5 sm:p-6 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-4">
              <div className="space-y-1">
                <div className="flex items-center space-x-2 text-xs font-bold text-amber-500">
                  <ShieldCheck className="w-4 h-4" />
                  <span>Шаг 2: Выбор корректного On-Call Playbook</span>
                </div>
                <h3 className="text-base sm:text-lg font-black text-slate-900 dark:text-white">
                  Какое действие устранит аварийную ситуацию в Production?
                </h3>
                <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                  Внимательно проанализируйте результаты терминала. Ошибка при выборе фикса может усугубить сбой.
                </p>
              </div>
                ...
                {currentScenario.fixOptions.map((fix) => {
                  ...
                  return (
                    <div
                      ...
                      className={`p-5 rounded-2xl border transition-all cursor-pointer space-y-2.5 relative ${
                        isChosen
                          ? fix.isCorrect
                    ...
                    >
                      ...
                      {isChosen && (
                        <div className={`p-3.5 rounded-xl text-xs font-medium leading-relaxed border animate-fadeIn ${
                          fix.isCorrect
                          ...
                          <strong className="font-bold block mb-0.5">
                            {fix.isCorrect ? '🎉 Авария успешно ликвидирована!' : '⚠️ Неверное решение:'}
                          </strong>
                          {fix.feedback}
                        </div>
                      )}
              ...
              {showSolution && (
                <div className="pt-4 border-t border-slate-200 dark:border-slate-800 flex justify-end">
                  <button
                    onClick={() => setActiveTab('postmortem')}
                    className="px-6 py-3 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black text-xs sm:text-sm transition-all shadow-md flex items-center space-x-2 cursor-pointer"
                  >
                    <span>Открыть Post-Mortem разбор</span>
                    <ArrowRight className="w-4 h-4" />
                  </button>
                </div>
              )}
          ...
          <div className="space-y-5 animate-fadeIn">
            <div className="bg-gradient-to-br from-emerald-950/40 via-slate-900 to-slate-950 border border-emerald-500/30 p-6 sm:p-8 rounded-3xl space-y-6 shadow-xl relative overflow-hidden">
              ...
              <div className="flex items-center justify-between border-b border-emerald-500/20 pb-4">
                ...
                <div className="px-3.5 py-1.5 rounded-xl bg-emerald-500 text-slate-950 font-black text-xs">
                  +150 XP ПОЛУЧЕНО
                </div>
              </div>
