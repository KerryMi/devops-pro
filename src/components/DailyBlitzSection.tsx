          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="space-y-1.5">
              <div className="flex items-center space-x-2 text-xs font-semibold text-slate-700 dark:text-slate-300">
                ...
                <span>Блиц-подборка на сегодня ({new Date().toLocaleDateString('ru-RU', { day: 'numeric', month: 'long' })}):</span>
              </div>
              <div className="flex flex-wrap gap-1.5 pt-1">
                {todayQuestions.map((q, i) => (
                  <span
                    key={q.id}
                    className="px-2.5 py-1 rounded-lg text-[11px] font-bold bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700"
                  >
            ...
            <button
              ...
              className="px-5 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-600 active:bg-amber-700 text-slate-950 font-extrabold text-xs transition-all shadow-md shadow-amber-500/20 flex items-center justify-center space-x-2 shrink-0"
            >
