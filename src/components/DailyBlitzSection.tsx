    ...
    <div className="h-full flex flex-col justify-between rounded-2xl bg-white dark:bg-[#121927] p-5 shadow-xs gap-4 transition-all">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2.5">
        ...
        <div className="flex items-center space-x-2 shrink-0 flex-wrap gap-y-1">
          <div className="px-2.5 py-1 rounded-xl bg-amber-500/10 text-amber-700 dark:text-amber-400 text-xs font-bold flex items-center space-x-1">
            <Flame className="w-3.5 h-3.5 fill-amber-500 text-amber-500" />
            <span>Стрик: <strong>{progress.dailyStreak || 0} дн.</strong></span>
          </div>

          {isCompletedToday && mode === 'idle' && (
            <div className="px-2.5 py-1 rounded-xl bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 text-xs font-extrabold flex items-center space-x-1">
          ...
          {mode === 'idle' && (
            <div className="px-2.5 py-1 rounded-xl bg-slate-100 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700/60 text-slate-600 dark:text-slate-300 font-mono text-[11px] font-extrabold flex items-center space-x-1.5">
        ...
        <div className="bg-slate-50 dark:bg-[#0b1120] p-4 rounded-xl space-y-3 my-auto flex flex-col justify-between">
          <div className="space-y-2">
            <div className="flex items-center justify-between text-xs font-bold text-slate-700 dark:text-slate-300 flex-wrap gap-1">
              ...
              <div className="flex items-center space-x-1 text-[11px] font-mono text-slate-500 dark:text-slate-400 font-semibold">
                <Clock className="w-3 h-3 text-amber-500" />
                <span>Обновление через {formattedTimer}</span>
              </div>
            </div>
          ...
          <div className="pt-1">
            {isCompletedToday ? (
              <div className="flex flex-col sm:flex-row items-center justify-between gap-2">
                <button
                  disabled
                  className="w-full sm:w-auto px-4 py-2 rounded-xl bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 font-extrabold text-xs flex items-center justify-center space-x-1.5 cursor-not-allowed"
                >
                  <CheckCircle className="w-4 h-4 text-emerald-500" />
                  <span>Пройдено сегодня ({todayResult?.score}/{todayResult?.total})</span>
                </button>
                <div className="flex items-center space-x-1.5 px-3 py-1.5 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-600 dark:text-amber-400 font-mono text-xs font-bold">
                  <Clock className="w-3.5 h-3.5 text-amber-500 animate-pulse" />
                  <span>Новый блиц через {formattedTimer}</span>
                </div>
              </div>
            ) : (
              <button
