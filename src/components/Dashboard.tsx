...
export const Dashboard: React.FC<DashboardProps> = ({
  ...
  const level4Status = totalQuizzesCount > 0
    ? { text: 'Запущено', style: 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20' }
    : hasLegend && solvedIncidentsCount > 0
      ? { text: 'Готов к прохождению', style: 'bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border border-indigo-500/20' }
      : { text: 'Заблокировано', style: 'bg-slate-100 dark:bg-slate-800 text-slate-500' };
  ...
  let advisorTip = "Запустите итоговый Собес-Симулятор для подтверждения квалификации!";
  if (masteredCount < 10) {
    advisorTip = "Решите новые вопросы на Уровне 1 (+10% к индексу)";
  } else if (!hasLegend) {
    advisorTip = "Заполните Легенду опыта на Уровне 2 (+15% к индексу)";
            ...
            <div className="grid grid-cols-2 gap-2 mt-3">
              {[
                ...
                },
                {
                  id: 'roadmap-level-4',
                  tab: 'quizzes',
                  title: '4. Собеседование',
                  progress: totalQuizzesCount > 0 ? `${totalQuizzesCount} раз` : 'Заблокировано 🔒',
                  percent: totalQuizzesCount > 0 ? 100 : 0,
                  icon: Target,
                  colorClass: totalQuizzesCount > 0 ? 'text-purple-500 bg-purple-500/10 border-purple-500/15' : 'text-slate-400 bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-800',
                  barColor: 'bg-purple-500',
                  statusText: totalQuizzesCount > 0 ? 'Доступно' : 'Заблокировано'
                }
              ].map((step) => {
        ...
        <div className="relative pl-8 sm:pl-10 space-y-8 before:absolute before:left-4 sm:before:left-5 before:top-4 before:bottom-4 before:w-0.5 before:bg-gradient-to-b before:from-emerald-500 before:via-blue-500 before:to-slate-300 dark:before:to-slate-800">
          ...
          <div id="roadmap-level-2" className="relative group">
            ...
            <div className={`absolute -left-8 sm:-left-10 top-5 w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center font-mono font-black text-xs sm:text-sm shadow-md transition-transform group-hover:scale-110 z-10 ${
              ...
              {hasLegend ? <CheckCircle2 className="w-4 h-4" /> : '2'}
            </div>
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-5 sm:p-6 rounded-2xl shadow-sm hover:border-emerald-500/40 transition-all">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Уровень 2 • Карьера</span>
                    <h3 className="text-base sm:text-lg font-bold text-slate-900 dark:text-white">
          ...
          <div id="roadmap-level-4" className="relative group">
            ...
            <div className={`absolute -left-8 sm:-left-10 top-5 w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center font-mono font-black text-xs sm:text-sm shadow-md transition-transform group-hover:scale-110 z-10 ${
              ...
              {totalQuizzesCount > 0 ? <CheckCircle2 className="w-4 h-4" /> : '4'}
            </div>
