...
import { RankAvatar } from '../widgets/RankAvatar';
import { 
  Award, 
  Zap, 
  BookOpen, 
...
export const AchievementsView: React.FC<AchievementsViewProps> = ({
  ...
  const dailyQuests = [
    ...
    {
      id: 'quest-4',
      title: 'Пройти Спринт-Тест Блица',
      description: 'Пройдите быстрый тест в игровом формате на главной панели',
      xpReward: 50,
      progressText: `${Object.keys(progress.dailyBlitzHistory || {}).length} / 1 блиц`,
    ...
    },
    {
      ...
      description: 'Пройдите любой тематический тест с результатом не менее 80% правильных ответов',
      xpReward: 100,
      progressText: `${(progress.quizResults || []).filter(r => r.totalQuestions > 0 && (r.score / r.totalQuestions) >= 0.8).length} / 1 тест`,
    ...
    },
    {
      ...
      description: 'Заполните и сохраните конструктор коммерческой легенды для резюме',
      xpReward: 100,
      progressText: progress.savedLegend ? 'Заполнено' : 'Не заполнено',
    ...
    <div className="max-w-5xl mx-auto space-y-8 pb-16 animate-fadeIn">
      ...
      <div className="bg-white dark:bg-gradient-to-br dark:from-slate-900 dark:via-[#121927] dark:to-slate-900 text-slate-800 dark:text-white border border-slate-200 dark:border-slate-800 p-6 sm:p-8 rounded-2xl shadow-xl relative overflow-hidden space-y-6">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 relative z-10">
          ...
          <div className="flex flex-col-reverse sm:flex-row items-start sm:items-center justify-between gap-6 flex-1 w-full">
            <div className="space-y-1.5 flex-1">
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-[10px] font-black uppercase tracking-wider px-2.5 py-0.5 rounded-full bg-amber-500/20 text-amber-500 dark:text-amber-400 border border-amber-500/40 font-mono whitespace-nowrap">
          ...
          {/* XP Stat Box & Action */}
          <div className="shrink-0 flex flex-col justify-between space-y-3 bg-slate-50 dark:bg-slate-800/80 p-5 rounded-2xl border border-slate-200 dark:border-slate-700/80 min-w-[240px]">
            ...
            <div className="space-y-1.5 pt-1">
              <div className="flex justify-between text-xs font-mono">
                <span className="text-slate-500 dark:text-slate-400">XP Уровня:</span>
                <span className="text-amber-600 dark:text-amber-400 font-bold">{currentXPInRank} / {xpSpanInRank}</span>
              </div>
              <div className="w-full h-3 bg-slate-100 dark:bg-slate-900 rounded-full overflow-hidden p-0.5 border border-slate-200 dark:border-slate-700">
                <div 
            ...
            <button
              ...
              className="w-full py-2 px-3 rounded-xl bg-slate-200/60 dark:bg-slate-700/60 hover:bg-slate-200 dark:hover:bg-slate-700 text-xs font-bold text-slate-700 dark:text-slate-200 flex items-center justify-center space-x-2 transition-all cursor-pointer"
            >
        ...
        {showRanksLadder && (
          <div className="pt-4 border-t border-slate-200 dark:border-slate-800 space-y-3 animate-fadeIn relative z-10">
            <h3 className="text-xs font-black uppercase tracking-widest text-amber-500 dark:text-amber-400 flex items-center space-x-1.5">
                ...
                return (
                  <div
                    ...
                    className={`p-3.5 rounded-xl border transition-all ${
                      isCurrent
                        ? 'bg-amber-500/10 border-amber-500/50 text-slate-950 dark:text-white ring-2 ring-amber-500/30'
                        : isPassed
                          ? 'bg-slate-50 dark:bg-slate-800/60 border-emerald-500/20 dark:border-emerald-500/30 text-slate-700 dark:text-slate-300'
                          : 'bg-slate-100/40 dark:bg-slate-900/40 border-slate-200 dark:border-slate-800 text-slate-400 dark:text-slate-500 opacity-60'
                    }`}
                  >
                      ...
                      <div className="flex items-center space-x-2.5">
                        ...
                        <div>
                          <div className="text-xs font-black flex items-center space-x-1">
                            ...
                            {isPassed && <Check className="w-3.5 h-3.5 text-emerald-500 dark:text-emerald-400 shrink-0" />}
                          </div>
                          <p className="text-[10px] text-slate-500 dark:text-slate-400 line-clamp-1 mt-0.5">{r.subtitle}</p>
                        </div>
                      </div>
                      <span className="text-[10px] font-mono font-bold px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 shrink-0">
                        {r.minXP} XP
                      </span>
                    </div>
                  </div>
