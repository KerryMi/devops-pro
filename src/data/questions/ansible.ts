...
*Единственный неидемпотентный модуль*: Модуль \`command\` / \`shell\` — запуск произвольного bash-скрипта всегда возвращает статус \`CHANGED\`, если не указаны параметры \`creates\` или \`changed_when\`.`,
    codeSnippet: {
      language: 'yaml',
...
*Плюс Kubespray*: Абсолютная гибкость, декларативное описание инфраструктуры кластера в Git (GitOps for K8s cluster).`,
  ...
  {
    id: 'ansible-15',
    title: 'Что такое Ansible Vault? Как безопасно управлять паролями и ключами в репозитории Ansible?',
    category: 'ansible',
    difficulty: 'Junior',
    summaryAnswer: 'Ansible Vault — встроенный инструмент шифрования (AES-256), позволяющий безопасно хранить пароли, токены и приватные ключи в файлах YAML прямо в репозитории Git.',
    fullAnswer: `Хранение секретов в чистом виде в публичном или приватном Git — критическое нарушение безопасности. Ansible Vault решает эту проблему, позволяя зашифровать как отдельные переменные, так и файлы целиком.

**Принцип работы**:
1. Вы создаете секретный файл или шифруете переменную с помощью пароля (Vault Password).
2. Ansible шифрует данные по алгоритму AES-256. В Git файл попадает в виде нечитаемого текста с заголовком \`$ANSIBLE_VAULT;1.1;AES256\`.
3. При запуске плейбука вы передаете пароль расшифровки через интерактивный ввод, файл с паролем (\`--vx-password-file\`) или переменную окружения.

**Лучшие практики разделения секретов**:
- Вместо шифрования всего файла \`group_vars/all.yml\` целиком (что затрудняет чтение диффов в Git), лучше зашифровать только секретные переменные с помощью \`ansible-vault encrypt_string\` и вставить зашифрованные строки прямо в обычный YAML-файл.
- Другой подход: разделить переменные на \`vars.yml\` (обычные) и \`vault.yml\` (полностью зашифрован, содержит секреты с префиксом \`vault_\`). В \`vars.yml\` ссылаются на них: \`db_password: "{{ vault_db_password }}"\`.`,
    codeSnippet: {
      language: 'bash',
      code: `# Зашифровать отдельную строку для вставки в YAML:
ansible-vault encrypt_string 'my-secret-db-password' --name 'vault_db_password'

# Запуск плейбука с чтением пароля расшифровки из файла (который добавлен в .gitignore!):
ansible-playbook -i hosts site.yml --vault-password-file .vault_pass`
    },
    interviewTips: [
      'Подчеркните, что файл с мастер-паролем (\`.vault_pass\`) ОБЯЗАТЕЛЬНО должен быть добавлен в \`.gitignore\`, чтобы случайно не залить его в репозиторий.'
    ],
    commonPitfalls: [
      'Шифровать большие конфигурационные файлы целиком, из-за чего теряется возможность отслеживать изменения настроек на Code Review.'
    ],
    tags: ['Ansible', 'Security', 'Vault', 'Secrets', 'BestPractices']
  },
  {
    id: 'ansible-16',
    title: 'Что такое Ansible Dynamic Inventory и для чего он используется в облачной инфраструктуре?',
    category: 'ansible',
    difficulty: 'Senior',
    summaryAnswer: 'Dynamic Inventory — механизм динамического формирования списка хостов на лету путем обращения через плагины или скрипты к внешним API (AWS EC2, GCP, VMware, NetBox) вместо ручного ведения статичного файла hosts.ini.',
    fullAnswer: `В динамических облачных или виртуализированных средах виртуальные машины постоянно создаются, удаляются или меняют IP-адреса (например, при автоскейлинге). Статичный файл \`hosts.yaml\` в таких условиях мгновенно устаревает.

**Как работает Dynamic Inventory**:
1. Ansible использует **Inventory Plugins** (предпочтительный подход) или исполняемые скрипты.
2. При запуске Ansible обращается к API провайдера (например, AWS) и запрашивает список запущенных инстансов.
3. Плагин фильтрует инстансы по тегам, регионам или статусу и возвращает JSON-структуру в формате инвентаря Ansible.
4. Вы можете группировать хосты на основе тегов (например, все машины с тегом \`Environment: Production\` и \`Role: Web\` автоматически попадут в группу \`web_prod\`).

**Примеры применения**:
- Автоматический деплой на новые ноды в AWS EC2.
- Синхронизация списка хостов с базой данных NetBox (Single Source of Truth для сетевой инфраструктуры).`,
    codeSnippet: {
      language: 'yaml',
      code: `# demo_aws_ec2.yml (пример конфигурации dynamic inventory для AWS)
plugin: amazon.aws.aws_ec2
regions:
  - us-east-1
keyed_groups:
  # Создать группы на основе тега 'Role'
  - key: tags.Role
    prefix: role`
    },
    interviewTips: [
      'Объясните разницу между старыми Dynamic Inventory Scripts (просто скрипты на Python/Bash, возвращающие JSON) и современными Inventory Plugins (нативные плагины Ansible, которые работают быстрее, поддерживают кэширование и описываются в YAML файлах).'
    ],
    commonPitfalls: [
      'Делать запросы к API облака на каждый запуск задачи в цикле без настройки кэширования инвентаря. Это может привести к блокировке запросов со стороны провайдера из-за Rate Limits (превышения лимитов запросов).'
    ],
    tags: ['Ansible', 'Inventory', 'Dynamic', 'AWS', 'GCP', 'Automation']
  }
];
