...
docker buildx build --platform linux/amd64,linux/arm64 \\
  ...
  {
    id: 'docker-31',
    title: 'Что такое Docker BuildKit и как работает продвинутое кэширование в нем?',
    category: 'docker',
    difficulty: 'Senior',
    summaryAnswer: 'BuildKit — это современный высокопроизводительный движок сборки образов в Docker. Он поддерживает параллельное выполнение независимых этапов сборки, монтирование кэшей (--mount=type=cache) для пакетных менеджеров и импорт/экспорт кэша в удаленный реестр.',
    fullAnswer: `BuildKit заменяет устаревший легаси-сборщик Docker. С версии Docker 23.0 движок BuildKit активирован по умолчанию.

**Ключевые преимущества и продвинутые фичи BuildKit**:
1. **Параллельное выполнение**: Если у вас в Multistage Dockerfile этапы сборки фронтенда и бэкенда независимы, BuildKit запустит их сборку одновременно, снижая общее время сборки.
2. **Монтирование кэша во время RUN (Cache Mounts)**:
   Традиционно, если меняется \`package.json\` или \`go.mod\`, Docker полностью сбрасывает кэш слоя с установкой зависимостей, скачивая все пакеты заново.
   BuildKit позволяет примонтировать постоянную директорию кэша пакетного менеджера (например, \`~/.npm\` или \`/go/pkg/mod\`) через \`RUN --mount=type=cache\`. Зависимости будут кэшироваться между РАЗНЫМИ сборками на этой машине, докачивая только новые пакеты!
3. **Безопасное монтирование секретов (Secret Mounts)**:
   Позволяет временно пробросить секретные ключи или SSH-ключи для скачивания приватных репозиториев без сохранения секретов в финальных слоях образа: \`RUN --mount=type=secret,id=mysecret\`.
4. **Удаленный экспорт кэша (Inline/Registry cache)**:
   Позволяет экспортировать метаданные кэша сборки прямо в Docker Registry. На серверах CI/CD (где кэш локально отсутствует) сборщик скачает только слои кэша и существенно ускорит пайплайн.`,
    codeSnippet: {
      language: 'dockerfile',
      code: `# syntax=docker/dockerfile:1
# Использование продвинутого кэширования для npm и Go зависимостей
FROM node:18-alpine AS build
WORKDIR /app
COPY package*.json ./
# Кэшируем папку .npm во время сборки
RUN --mount=type=cache,target=/root/.npm npm ci
COPY . .
RUN npm run build`
    },
    interviewTips: [
      'Упомяните, что для использования расширенного синтаксиса (например, монтирования кэша или секретов) в самом верху Dockerfile пишется специальная директива: \`# syntax=docker/dockerfile:1\`.'
    ],
    commonPitfalls: [
      'Использовать стандартные переменные окружения \`ARG\` или \`ENV\` для передачи паролей/секретов сборки — эти значения навсегда оседают в истории слоев (\`docker history\`) и могут быть легко извлечены злоумышленниками.'
    ],
    tags: ['Docker', 'BuildKit', 'Caching', 'Dockerfile', 'Optimization']
  },
  {
    id: 'docker-32',
    title: 'Как использовать многоэтапную сборку (Multistage Builds) для уменьшения размера и повышения безопасности образов?',
    category: 'docker',
    difficulty: 'Middle',
    summaryAnswer: 'Multistage Builds разделяет процесс сборки приложения на несколько этапов (stages) в рамках одного Dockerfile. Это позволяет использовать тяжелое SDK/инструменты компиляции на первом этапе, а в финальный образ скопировать только готовый скомпилированный бинарник или статику.',
    fullAnswer: `Без многоэтапной сборки разработчикам приходилось либо оставлять все инструменты сборки (компиляторы, git, исходный код) в продакшн-образе (что раздувало его размер до гигабайтов и увеличивало вектор атак), либо писать сложные внешние bash-скрипты.

**Принцип работы Multistage**:
1. Вы описываете первый этап (например, \`FROM golang:1.20 AS builder\`), где устанавливаете зависимости, утилиты тестирования и компилируете бинарник.
2. Описываете второй, минимальный этап (например, \`FROM alpine:latest\` или \`FROM gcr.io/distroless/static\`).
3. Копируете скомпилированный файл из первого этапа с помощью флага \`COPY --from=builder /app/main /main\`.
4. В результате все исходники, компиляторы и утилиты остаются на промежуточном этапе и выбрасываются. Итоговый образ содержит ровно один легковесный файл.

**Преимущества для безопасности**:
- **Уменьшение Attack Surface (поверхности атаки)**: В итоговом образе нет пакетных менеджеров (apt, apk), компиляторов и шеллов (в случае \`distroless\` образов), что делает невозможным выполнение вредоносных скриптов злоумышленником при взломе приложения.`,
    codeSnippet: {
      language: 'dockerfile',
      code: `# ЭТАП 1: Компиляция приложения Go
FROM golang:1.20-alpine AS builder
WORKDIR /src
COPY . .
RUN CGO_ENABLED=0 GOOS=linux go build -o myapp .

# ЭТАП 2: Минимальный продакшн-образ
FROM alpine:3.18
...
ENTRYPOINT ["./myapp"]`
    ...
    interviewTips: [
      'Расскажите про "distroless"-образы (от Google). Они вообще не содержат shell-оболочки (bash/sh), пакетных менеджеров и утилит, только рантайм приложения и базовые SSL-сертификаты. Это стандарт максимальной безопасности.'
    ],
    commonPitfalls: [
      'Копировать исходный код и зависимости со всего builder-образа во второй этап вместо копирования только скомпилированного артефакта, сводя на нет преимущества Multistage.'
    ],
    tags: ['Docker', 'Dockerfile', 'Multistage', 'Security', 'Optimization']
  }
];
